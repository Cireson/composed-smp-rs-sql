...
if($gmsa_username -ne "_"){

$Usernames = $gmsa_username -split ","

Foreach($User in $Usernames){
$Query = @"
USE [master]; CREATE User "$User" EXEC sp_addsrvrolemember @loginame = N'$User',  @rolename = N'sysadmin';
"@
Try{
    invoke-sqlcmd -Query $Query 
    Write-Host "Added $User as SysAdmin to SQL Container"
    }
Catch{
    write-host "$_"
    }
